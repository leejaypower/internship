...
const {
  reservationRepository, rentalRepository,
...
const createRentalStart = async (userId, createRentalData) => {
  ...
  const rentalableNum = BUSINESS.MAX_RENTAL_NUM - user.rentalCount;
  if (rentalableNum <= 0) {
  ...
  // 예약이 되어있는 도서인지, 예약자는 누구인지 확인
  // 도서 예약이 되어 있고, 해당 예약자가 아니라면, error
  const reservation = await reservationRepository.getNextReservationByBookInfo(bookInfoId);
  if (reservation.length > 0 && reservation[0].userId !== userId) {
...
const createRentalExtend = async (userId, createRentalData) => {
  ...
  const extendCount = rentalHistory
    .filter((rental) => rental.state === TABLE.RENTAL_STATE.EXTEND).length;

  if (extendCount >= BUSINESS.MAX_EXTEND_COUNT) {
  ...
  const reservationCount = await reservationService.countByBookInfo({
  ...
  // 연장 진행
  const rentalStart = rentalHistory.filter((rental) => rental.state === TABLE.RENTAL_STATE.START)[0];
  const createRentalExtendData = {
    userId,
    bookId: rentalStart.bookId,
    state,
